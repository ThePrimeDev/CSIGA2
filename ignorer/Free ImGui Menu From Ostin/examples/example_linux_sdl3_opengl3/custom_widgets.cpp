...
bool color_button(const char* desc_id, const ImVec4& col)
{
    ...
    window->DrawList->AddText(set->widgets_font, set->widgets_font->LegacySize, rect.Min - ImVec2(1, -1), gui->get_clr(clr->color_label), desc_id);

    window->DrawList->AddText(set->value_font, set->value_font->LegacySize, ImVec2(rect.Max.x - 28 - set->value_font->CalcTextSizeA(set->value_font->LegacySize, FLT_MAX, -1.f, imvec4_to_hex(col).c_str()).x, rect.Min.y + 2), gui->get_clr(clr->color_label), imvec4_to_hex(col).c_str());
...
bool pipette_button(std::string_view label, bool* callback)
{
    ...
    window->DrawList->AddRectFilled(rect.Min, rect.Max, gui->get_clr(clr->accent, state->alpha), set->widgets_rounding);
    window->DrawList->AddText(set->colorpicker_icon_font, set->colorpicker_icon_font->LegacySize, rect.Min + ImVec2(7, 7), gui->get_clr(state->icon_clr), label.data());
...
bool c_widgets::color_edit(std::string_view label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if ((ItemHoverable(g.LastItemData.Rect, g.LastItemData.ID, g.LastItemData.ItemFlags) && g.IO.MouseClicked[0]) || (state->active && g.IO.MouseClicked[0] && !state->hovered && !state->pipette_active))
        ...
        {
            ImGui::Begin("picker_window", NULL, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove);
            {
...
#else
                    // Screen color picking is Windows-only in this sample.
#endif
